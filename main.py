...
def main():
    ...
    for i, color_input in enumerate(st.session_state.color_inputs):
        with st.container():
            ...
            name = st.text_input(f"Color Name {i + 1}", color_input["name"], key=f"name_{i}")
            hex_color = st.color_picker(f"Select Color {i + 1}", color_input["hex"], key=f"color_{i}")
            ...
            st.session_state.color_inputs[i]["name"] = name
            st.session_state.color_inputs[i]["hex"] = hex_color

            if is_valid_hex(hex_color):
                palette = generate_palette(hex_color, name)
                
                # Display color palette
                st.write(f"### {name} Palette")
